        ...
        ticketKey match {
          ...
          case _ => {
            while (flightStream.next()) {
              ...
              if(!authProvider.checkPermission(authenticatedUser.get, dfName, List.empty.asJava))
  ...
  override def getFlightInfo(context: FlightProducer.CallContext, descriptor: FlightDescriptor): FlightInfo = {
      ...
      val schema =  if (request != null) {
        ...
        if (dataFrameSchema == StructType.empty) {
          throw new StatusRuntimeException(io.grpc.Status.NOT_FOUND.withDescription("DataFrame not exist!"))
        }
        else
