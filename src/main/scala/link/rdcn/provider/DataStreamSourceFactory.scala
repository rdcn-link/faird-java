    ...
    val stream: Iterator[Row] = inputSource match {
      ...
      case DirectorySource(false) =>
        ...
        DataUtils.listFilesWithAttributes(dataFrameInfo.name).toIterator.zipWithIndex
          ...
          .map{case (file, index) => (index, file._1.getName, file._2.size(), DataUtils.getFileTypeByExtension(file._1), file._2.creationTime(), file._2.lastModifiedTime(), file._2.lastAccessTime(),file._1)}
          .map(Row.fromTuple(_))
    }
